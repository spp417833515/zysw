...
class Employee(Base):
    ...
    base_salary: Mapped[float] = mapped_column(Numeric(12, 2), default=0)  # 基本工资
    pay_day: Mapped[int] = mapped_column(Integer, default=15)  # 每月发薪日
    # 五险一金（个人部分比例，默认0 - 一人公司可不缴）
    social_insurance_rate: Mapped[float] = mapped_column(Numeric(6, 4), default=0)  # 社保个人比例 %
...
class SalaryRecord(Base):
    ...
    __table_args__ = (
        UniqueConstraint("employee_id", "year", "month", name="uq_salary_employee_year_month"),
    )
